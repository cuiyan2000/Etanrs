...
class BlackBoxAdvTrainer:
    ...
    def _initialize(self, train_data):
        ...
        # self.fake_tensor1 = self.fake_tensor
        # self.momentum=torch.zeros_like(self.fake_tensor).detach()
        # self.alpha = 0.06 / 30
        # self.p = np.inf
        # self.epsilon=0.06
        # self.grad=torch.zeros_like(self.fake_tensor).detach()
        # self.fake_tensor1.requires_grad_()  

        self.optimizer = optim.SGD([self.fake_tensor],lr=self.args.adv_lr,momentum=self.args.adv_momentum,nesterov=True)
    ...
    def train_epoch(self, train_data, epoch_num):
        ...
        def _compute_adv_grads():
            ...
            # self.fake_tensor2 = self.fake_tensor1 + self.momentum * self.args.decay_factor * self.alpha
            # data_tensor = torch.cat([sparse2tensor(train_data).to(self.device), self.fake_tensor2.detach().clone().to(self.device)], dim=0)

            data_tensor = torch.cat([sparse2tensor(train_data).to(self.device),self.fake_tensor.detach().clone().to(self.device)], dim=0)
        ...
        if self.args.attack_type == "random":
            ...
            # normalized_adv_grads=normalized_adv_grads + self.momentum*self.args.decay_factor
            # self.momentum=normalized_adv_grads

            # if self.fake_tensor.grad is None:
            #     # self.fake_tensor.grad = normalized_adv_grads.detach().cpu() * self.alpha
            #     if self.p==np.inf:
            #         updates = normalized_adv_grads.sign()
            #     else:
            #         normVal = torch.norm(normalized_adv_grads.view(normalized_adv_grads.shape[0], -1), self.p, 1,keepdim=True)
            #         updates = normalized_adv_grads/normVal
            #     updates = updates*self.alpha

            #     self.fake_tensor1 = self.fake_tensor1 +updates

            #     # project the disturbed image to feasible set if needed
            #     delta = self.fake_tensor1 -self.fake_tensor
            #     if self.p==np.inf:
            #         delta = torch.clamp(delta, -self.epsilon, self.epsilon)
            #     else:
            #         normVal = torch.norm(delta.view(delta.shape[0], -1), self.p, 1,keepdim=True)
            #         mask = normVal<=self.epsilon
            #         scaling = self.epsilon/normVal
            #         scaling[mask] = 1
            #         delta = delta*scaling
            #     self.fake_tensor1 = self.fake_tensor+delta
            # else:
            #     if self.p==np.inf:
            #             updates = normalized_adv_grads.sign()
            #     else:
            ...
            # if self.fake_tensor.grad is None:
            #     self.fake_tensor1.grad = normalized_adv_grads.detach().cpu() * self.alpha
            # else:
            #     self.fake_tensor1.grad.data = normalized_adv_grads.detach().cpu()
            #     self.fake_tensor1 = self.fake_tensor1 +updates
            #     delta = self.fake_tensor1 -self.fake_tensor
            #     if self.p==np.inf:
            #         delta = torch.clamp(delta, -self.epsilon, self.epsilon)
